...
- 이미지는 <img src="" data-img-slot="hero|about|service|gallery|blog"> 로 빈 src + data 속성만 추가.
...
[절대 규칙 — 텍스트는 무조건 플레이스홀더]
스크린샷에 보이는 텍스트를 절대 그대로 쓰지 말 것.
모든 텍스트는 아래 형식으로만 작성:
...
{{SUBTITLE}}        → 서브 슬로건
{{ABOUT_DESC}}      → 소개 설명
{{SERVICE_1}} ~ {{SERVICE_5}} → 서비스명
{{SERVICE_DESC_1}} ~ {{SERVICE_DESC_5}} → 서비스 설명 (각각 다른 내용으로)
{{PHONE}}           → 전화번호
{{ADDRESS}}         → 주소
{{HOURS}}           → 영업시간
{{STAT_1}} ~ {{STAT_3}} → 통계 숫자
{{STAT_LABEL_1}} ~ {{STAT_LABEL_3}} → 통계 레이블
{{NAV_1}} ~ {{NAV_7}} → 네비게이션 메뉴명
{{CTA_TEXT}}        → CTA 버튼 텍스트

위 플레이스홀더 이외의 실제 텍스트(업체명, 전화번호, 슬로건, 서비스명 등)를 절대 하드코딩하지 마세요.

[섹션별 최소 보장]
- Nav: 로고({{BRAND_NAME}}) + 메뉴({{NAV_1}}~{{NAV_5}}) + CTA 버튼({{CTA_TEXT}})
- Hero: 풀스크린 배경이미지 + 제목({{TAGLINE}}) + 부제목({{SUBTITLE}}) + CTA 버튼({{CTA_TEXT}})
- Services: 카드 또는 탭 형태로 {{SERVICE_1}}~{{SERVICE_5}} 목록 (각각 이미지+제목+설명)
- Contact: {{PHONE}} + {{ADDRESS}} + {{HOURS}} + 상담 폼(이름/연락처/문의내용/제출버튼)
...
function removeStyles(html: string): string {
  ...
  // 단독 <head> / </head> 태그도 제거 (짝이 안 맞는 경우)
