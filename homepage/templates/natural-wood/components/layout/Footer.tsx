        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              {company.instagram && (
                <a href={company.instagram} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-2xl bg-surface border border-border hover:border-primary transition-colors">
                  <Instagram className="h-5 w-5 text-text-secondary" />
                </a>
              )}
              {company.youtube && (
                <a href={company.youtube} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-2xl bg-surface border border-border hover:border-primary transition-colors">
                  <Youtube className="h-5 w-5 text-text-secondary" />
                </a>
              )}
              {company.naverPlace && (
                <a href={company.naverPlace} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-2xl bg-surface border border-border hover:border-primary transition-colors text-xs font-bold text-text-secondary">
                  N
                </a>
              )}
            </div>
