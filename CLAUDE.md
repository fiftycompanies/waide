...
| `generate/brand-injector.ts` | 의미 기반 플레이스홀더 + data-img-slot → Unsplash + bg-[url()] 교체 + 국기 이모지 + sanitize + replaceEmptyBoxes (서비스 아코디언 주입) + enforceDarkTone (Blog/Footer 다크 통일) + wrapNavInFlexContainer (Hero Nav flex 래퍼 자동 복구) | 활성 |
