...
- Phase SCREENSHOT-TO-CODE: Screenshot-to-Code 방식 도입 — HTML 크롤링 완전 폐기 (2026-03-20)
  ...
  - **Vision 2단계 분리 + 의미 기반 매핑 (2026-03-20)**:
    - vision-to-html.ts 전면 재작성: Stage A (구조 분석 JSON, multi-image Vision) + Stage B (섹션별 HTML 생성, 3개씩 병렬)
    - brand-injector.ts 전면 재작성: 의미 기반 플레이스홀더 (ITEM_TITLE/FORM_NAME_LABEL/FOOTER_COL_TITLE 등 분리)
    - 기본 footer 자동 추가 (Stage A에서 footer 미감지 시 {{PHONE}}/{{ADDRESS}}/{{COPYRIGHT}} 포함 footer 자동 추가)
  ...
  - **Tailwind CSS + 크롭 기반 전환 (2026-03-20)**:
    ...
    - brand-injector.ts: replaceTailwindBgUrls() (Tailwind bg url 임의값 → Unsplash 교체)
  ...
  - **노이즈 제거 + 다크 톤 통일 (2026-03-20)**:
    - vision-to-html.ts: buildCropUserPrompt()에 노이즈 금지 규칙 추가 (워터마크 문자/빈 박스/화이트 배경 금지)
    - vision-to-html.ts: getDefaultBlogHtml()/getDefaultFooterHtml() 다크 테마 (#1A1A1A/#111111)
    - brand-injector.ts: replaceEmptyBoxes() — 빈 배경색 div 감지 → 서비스 아코디언 주입
    - brand-injector.ts: buildServiceAccordionHtml() — 베이지(#F0EDE8) 서비스 리스트 컴포넌트
    - brand-injector.ts: enforceDarkTone() — Blog bg-white→#1A1A1A, Footer bg-gray-900→#111111
  - **Hero Nav flex 컨테이너 복구 (2026-03-20)**:
    - brand-injector.ts: wrapNavInFlexContainer() — Hero section 내부 nav가 flex 부모 없이 나열된 경우 absolute top-0 flex justify-between 래퍼로 자동 감싸기
    - 기존 flex 컨테이너가 있으면 스킵 (이중 래핑 방지)
  - tsc --noEmit 통과
- Phase TEMPLATE-INJECT: 사전 제작 템플릿 주입 파이프라인 (2026-03-20)
  - **핵심 원칙**: "고품질 사전 제작 HTML 템플릿 3종 + AI 슬롯 주입" — 레퍼런스 복제 방식과 병렬 운영
  - **기존 코드 무변경**: 새 파일 + 함수 추가만, DOM 복제/Screenshot-to-Code 파이프라인 동작 영향 없음
  - lib/homepage/templates/: HTML 슬롯 템플릿 3종 + 미리보기 3종
    - dark-luxury.html: 의료/뷰티 (#1a1a1a + #c8a97e gold, Bebas Neue, 8섹션)
    - warm-natural.html: 숙박/캠핑 (#fefcf9 cream + #4a7c59 forest green, Noto Serif KR, 9섹션+리뷰)
    - light-clean.html: 카페/인테리어 (white + CSS var accent color, Pretendard, 8섹션+탭+포트폴리오)
  - lib/homepage/generate/template-types.ts (신규): TemplateName, TEMPLATE_NAMES, TEMPLATE_LABELS, TemplateSlotContent (클라이언트 안전 공용 타입)
  - lib/homepage/generate/brand-content-generator.ts (신규): generateBrandContent() — 브랜드 데이터 → 슬롯 콘텐츠 생성 (공통 룰 기반 + Unsplash 이미지 + Claude Haiku 4.5 템플릿 전용 슬롯)
  - lib/homepage/generate/brand-injector.ts: injectToTemplate() 함수 추가 — {{SLOT}} 플레이스홀더 교체 (기존 injectBrandInfo 무변경)
  - lib/homepage/generate/homepage-generator.ts: generateFromTemplate() 메서드 추가 — 템플릿 로드 → 슬롯 생성 → 주입 → DB 저장 → 배포
  - lib/actions/homepage-generate-actions.ts: templateName 파라미터 분기 (template → generateFromTemplate, 없으면 기존 generate)
  - components/homepage/generate-pipeline-form.tsx: 생성 방식 선택 UI (복제 vs 템플릿) + 템플릿 드롭다운
  - homepage_projects.template_id: "dark-luxury" | "warm-natural" | "light-clean" (기존 "clone" 병렬)
  - homepage_projects.theme_config: pipeline="template-injection"
  - tsc --noEmit 통과
- Phase HOMEPAGE-REQUEST: 포털 제작 신청 + 어드민 관리 파이프라인 (2026-03-20)
  - **기존 코드 무변경**: 새 파일 추가 + 최소 수정만, 기존 기능 동작 영향 없음
  - scripts/migrations/070_homepage_requests.sql: homepage_requests 테이블 (status CHECK: pending/generating/completed/delivered/failed, RLS 활성화)
  - lib/actions/homepage-request-actions.ts: 포털 제작 신청 + 어드민 목록/상태/통계 서버 액션
  - lib/homepage/generate/template-types.ts: TemplateName에 "3d-glamping" 추가 + TEMPLATE_DESCRIPTIONS/TEMPLATE_BG_COLORS 상수
  - components/homepage/client-homepage-view.tsx: 프로젝트 없는 고객에게 4종 템플릿 선택 카드 + 제작 신청 + 진행 상태 + 신청 이력 표시
  - app/(dashboard)/homepage/page.tsx: ClientHomepageView에 clientId prop 전달
  - app/(dashboard)/ops/homepage-requests/page.tsx: 어드민 신청 관리 (통계 5종 + 상태 필터 + [생성] → generateFromTemplate() + [전달] + 프로젝트 연결)
  - components/dashboard/app-sidebar.tsx: 홈페이지 그룹에 "신청 관리" 메뉴 추가 (/ops/homepage-requests)
  - tsc --noEmit 통과
...
| 24 | **TEMPLATE-INJECT** | 사전 제작 템플릿 주입 — 3종 HTML 템플릿 + AI 슬롯 주입 + 어드민 UI 템플릿 선택 | ✅ 완료 |
| 25 | **HOMEPAGE-REQUEST** | 포털 제작 신청 + 어드민 관리 — 템플릿 선택 UI + homepage_requests 테이블 + 생성 파이프라인 연동 | ✅ 완료 |
...
| 070 | homepage_requests 테이블: 포털 제작 신청 (client_id FK, template_name, status CHECK: pending/generating/completed/delivered/failed, project_id FK → homepage_projects, requested_by FK → users, RLS) | **SQL 생성 완료** |

> ⚠️ 045~070: scripts/migrations/ 디렉토리에 SQL 파일 생성. Supabase Dashboard에서 실행 필요.
