...
export default function PortalReportsContent() {
  ...
  const agentNames: Record<string, string> = {
    CMO: "CMO (전략)", RND: "RND (분석)", COPYWRITER: "COPYWRITER (작성)",
    QC: "QC (검수)", ANALYST: "ANALYST (분석봇)", PUBLISHER: "PUBLISHER (발행)",
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          onClick={() => window.open(`/api/portal/report-pdf?month=${selectedYear}-${String(selectedMonth).padStart(2, "0")}`, "_blank")}
          className="flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm font-medium text-gray-700 hover:bg-gray-50"
        >
        ...
        <select
          ...
          className="px-3 py-2 rounded-lg border text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500"
        >
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { icon: FileText, color: "text-purple-500", label: "발행 콘텐츠", value: data.summary.monthlyContents },
          { icon: Key, color: "text-emerald-500", label: "신규 키워드", value: data.summary.newKeywords },
          { icon: Sparkles, color: "text-violet-500", label: "AI 실행", value: data.summary.agentExecutions },
        ].map((card) => (
          <div key={card.label} className="p-5 rounded-xl border bg-white text-center">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart
                data={data.contentsTrend.map((ct) => {
                  ...
                  return { month: ct.month, publishCount: ct.count, activeKeywords: kw?.count ?? 0 };
                })}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="month" tickFormatter={formatMonth} tick={{ fontSize: 12, fill: "#9ca3af" }} />
                <YAxis yAxisId="left" allowDecimals={false} tick={{ fontSize: 12, fill: "#9ca3af" }} />
                <YAxis yAxisId="right" orientation="right" allowDecimals={false} tick={{ fontSize: 12, fill: "#9ca3af" }} />
      ...
      <div className="rounded-xl border bg-white p-6">
        ...
        {data.contentsTrend.every((d) => d.count === 0) ? (
          <p className="text-sm text-gray-400 text-center py-8">아직 발행 데이터가 없습니다</p>
        ) : (
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
                ...
                {data.serpRankings.map((r, i) => (
                  <tr key={i} className="border-b last:border-0">
                    ...
                    <td className="py-2 px-3 text-center">
                      {r.rank > 0 ? (
                        <span className={`font-bold ${r.rank <= 3 ? "text-emerald-600" : r.rank <= 10 ? "text-blue-600" : r.rank <= 20 ? "text-amber-600" : "text-gray-400"}`}>
                          {r.rank}위
                        </span>
                    ...
                    <td className="py-2 px-3 text-center">
                      {r.rank_google != null ? (
                        <span className={`font-bold ${r.rank_google <= 3 ? "text-emerald-600" : r.rank_google <= 10 ? "text-blue-600" : r.rank_google <= 20 ? "text-amber-600" : "text-gray-400"}`}>
                          {r.rank_google}위
                        </span>
                      ) : <span className="text-gray-300">-</span>}
                    </td>
        ...
        <div className="rounded-xl border bg-white p-6">
          ...
          <div className="flex items-center gap-6">
            <div>
              ...
              <p className="text-4xl font-bold text-purple-600">{data.aeo.score}</p>
            </div>
          </div>
