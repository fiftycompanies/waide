...
### D-3.1: SEO 키워드 매트릭스 데이터 파일
- **설명**: 인테리어 업종 특화 키워드 매트릭스를 정의하는 데이터 파일. 132개 지역(서울 25구 + 경기 31시 + 기타 광역시/도) x 124개 키워드 패턴(기본 서비스 20개, 공간별 15개, 유형별 20개, 비용 관련 15개, 후기 관련 10개, 트렌드 15개, 질문형/AEO 15개, 비교형 14개)의 조합을 정의한다.
- **파일**:
  - `lib/homepage/seo-keyword-matrix.ts` (키워드 카테고리 및 패턴)
  - `lib/homepage/regions.ts` (132개 지역 데이터)
  - `lib/homepage/keyword-patterns.ts` (124개 키워드 패턴)
- **검증 방법**: 지역 수 132개, 패턴 수 124개 확인, 각 패턴의 source/priority/metadata 필드 존재 확인

### D-3.2: generateHomepageKeywords() Server Action
- **설명**: 프로젝트의 서비스 지역과 서비스 유형을 기반으로 키워드 매트릭스에서 키워드를 자동 생성하여 keywords 테이블에 일괄 INSERT하는 Server Action. 중복 체크, 대표 키워드 설정, 배치 처리(100개 단위)를 포함한다.
- **파일**: `lib/homepage/actions/generate-keywords.ts`
- **검증 방법**: 지역 3개 입력 시 올바른 수의 키워드 생성 확인, 중복 재실행 시 0개 생성 확인, is_primary 설정 확인

### D-3.3: 자동/수동 키워드 생성 트리거
- **설명**: 자료 수집 폼 STEP 2(서비스 정보) 완료 후 자동 키워드 생성을 트리거하는 훅과, 프로젝트 상세에서 수동으로 "키워드 재생성" 버튼을 제공하는 UI 컴포넌트.
- **파일**:
  - `app/(dashboard)/dashboard/homepage/[id]/_components/keyword-generator.tsx`
  - `app/(dashboard)/dashboard/homepage/[id]/_components/keyword-preview.tsx`
  - `app/(dashboard)/dashboard/homepage/[id]/_components/keyword-result.tsx`
- **검증 방법**: 자료 수집 완료 시 키워드 자동 생성 확인, 수동 재생성 버튼 클릭 시 동작 확인

### D-3.4: 기존 키워드 관리 페이지 필터 확장
- **설명**: 기존 `/dashboard/keywords` 페이지의 source 필터에 'homepage_seo', 'blog_target' 옵션을 추가하고, blog_type 컬럼(정보성/후기성/AEO)을 키워드 테이블에 표시한다.
- **파일**: `app/(dashboard)/dashboard/keywords/_components/keyword-filters.tsx` (수정)
- **검증 방법**: source 필터 'homepage_seo', 'blog_target' 선택 시 해당 키워드만 표시, blog_type 컬럼 표시 확인

---

## 인수 기준 (Acceptance Criteria)

### AC-3.1: 키워드 매트릭스 데이터 완전성
- [ ] 서울 25개구 데이터 포함
- [ ] 경기 31개시 데이터 포함
- [ ] 기타 광역시/도 지역 데이터 포함 (총 132개 이상)
- [ ] 키워드 패턴 124개 이상 정의
- [ ] 각 패턴에 template, priority, source, metadata(해당 시) 필드 존재

### AC-3.2: 키워드 생성 정확성
- [ ] 지역과 패턴의 교차 조합이 올바르게 생성됨 ({region} 치환 정상)
- [ ] source 필드가 올바르게 할당됨 (homepage_seo 또는 blog_target)
- [ ] priority 필드가 올바르게 할당됨 (critical, high, medium)
- [ ] metadata.blog_type이 올바르게 할당됨 (정보성, 후기성, AEO)
- [ ] metadata.region, metadata.generated_by, metadata.project_id 포함

### AC-3.3: 중복 방지
- [ ] 동일 client_id + keyword 조합의 중복 키워드를 생성하지 않음
- [ ] 재실행 시 기존 키워드를 건너뛰고 신규 키워드만 INSERT
- [ ] duplicatesSkipped 카운트 정확

### AC-3.4: 대표 키워드 설정
- [ ] 첫 번째 critical 우선순위 키워드에 is_primary=true 설정
- [ ] 나머지 키워드는 is_primary=false

### AC-3.5: 배치 처리 성능
- [ ] 500개 이상 키워드 일괄 생성 시 타임아웃 없이 완료
- [ ] 100개 단위 배치 INSERT 정상 동작

### AC-3.6: 기존 시스템 연동
- [ ] keywords 테이블의 기존 컬럼 구조와 호환
- [ ] 기존 키워드 관리 페이지에서 신규 키워드 조회 가능
- [ ] source 필터에 'homepage_seo', 'blog_target' 옵션 추가됨

---

## 테스트 요구사항 (Test Requirements)

### T-3.1: 키워드 매트릭스 데이터 검증
- **유형**: 단위
- **설명**: REGIONS 객체의 지역 수 합산이 132개 이상인지, KEYWORD_CATEGORIES의 전체 패턴 수가 124개 이상인지 확인한다.
- **예상 결과**: 지역 수 >= 132, 패턴 수 >= 124.

### T-3.2: generateHomepageKeywords() 정상 생성 테스트
- **유형**: 통합
- **설명**: service_regions = ['강남구', '서초구', '송파구']인 프로젝트에 대해 generateHomepageKeywords()를 실행하고, 생성된 키워드 수와 카테고리별 분포를 확인한다.
- **예상 결과**: totalGenerated > 0, 카테고리별 카운트 정확, primaryKeyword가 '{지역} 인테리어' 형식.

### T-3.3: 중복 스킵 테스트
- **유형**: 통합
- **설명**: 동일 프로젝트에 대해 generateHomepageKeywords()를 2회 실행하고, 2회차 실행 결과를 확인한다.
- **예상 결과**: 2회차 totalGenerated = 0, duplicatesSkipped > 0.

### T-3.4: 배치 INSERT 성능 테스트
- **유형**: 통합
- **설명**: service_regions에 10개 이상 지역을 설정하고 generateHomepageKeywords()를 실행하여 500개 이상 키워드 생성 성능을 확인한다.
- **예상 결과**: 30초 이내 완료, 모든 키워드 정상 INSERT.

### T-3.5: 자동 트리거 테스트
- **유형**: E2E
- **설명**: 자료 수집 위저드 STEP 2 완료 후 completeCollection() 호출 시 generateHomepageKeywords()가 자동 실행되는지 확인한다.
- **예상 결과**: 키워드 자동 생성, keywords 테이블에 데이터 존재.

### T-3.6: 수동 재생성 테스트
- **유형**: E2E
- **설명**: 프로젝트 상세 페이지에서 "키워드 재생성" 버튼 클릭 시 UI에 진행률과 결과가 표시되는지 확인한다.
- **예상 결과**: 로딩 상태 표시, 완료 후 생성 결과(총 수, 카테고리별 수, 중복 수) 표시.

### T-3.7: 키워드 관리 페이지 필터 테스트
- **유형**: E2E
- **설명**: /dashboard/keywords 페이지에서 source 필터를 'homepage_seo'로 변경하고 해당 키워드만 표시되는지 확인한다.
- **예상 결과**: homepage_seo 소스 키워드만 목록에 표시.

---

## 의존성 (Dependencies)
- Phase 1: keywords 테이블 존재, metadata JSONB 컬럼 활용 가능
- Phase 2: 자료 수집 폼에서 service_regions, service_types 데이터 입력 가능, completeCollection() 액션 존재

## 위험 요소 (Risks)
- **대량 키워드 생성 시 DB 부하**: 지역 10개 x 패턴 124개 = 1,240개 키워드 일괄 INSERT 시 DB 부하 가능. 대응: 100개 단위 배치 처리, 에러 발생 시 부분 롤백 방지.
- **키워드 패턴의 지역 독립성**: 일부 패턴은 '{region}' 없이 일반적인 키워드(예: "인테리어 업체 고르는 방법")인데, 모든 지역에 대해 중복 생성될 수 있음. 대응: region 플레이스홀더가 없는 패턴은 1회만 생성하도록 분기 처리.
- **기존 키워드와의 충돌**: 기존에 수동으로 등록된 키워드와 자동 생성 키워드가 겹칠 수 있음. 대응: INSERT 전 기존 키워드 조회로 중복 체크.
...
> 아래 내용은 `/docs/phases/phase-3-detail.md`에서 통합되었습니다.
