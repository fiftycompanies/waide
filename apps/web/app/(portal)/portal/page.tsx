    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="p-4 rounded-xl border bg-white">
          ...
          {kpi.suggestedKeywords > 0 && (
            <p className="text-[10px] text-violet-600 mt-0.5">승인 대기 중</p>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <Link
          ...
          className="flex flex-col items-center gap-1.5 p-3 rounded-xl border bg-white hover:bg-emerald-50 hover:border-emerald-200 transition-colors"
        >
          <Key className="h-5 w-5 text-emerald-600" />
          <span className="text-xs font-medium text-gray-700">키워드 관리</span>
        </Link>
        <Link
          href="/portal/blog?tab=contents"
          className="flex flex-col items-center gap-1.5 p-3 rounded-xl border bg-white hover:bg-purple-50 hover:border-purple-200 transition-colors"
        >
          <FileText className="h-5 w-5 text-purple-600" />
          <span className="text-xs font-medium text-gray-700">콘텐츠 현황</span>
        </Link>
        <Link
          href="/portal/blog"
          className="flex flex-col items-center gap-1.5 p-3 rounded-xl border bg-white hover:bg-blue-50 hover:border-blue-200 transition-colors"
        >
          <Rss className="h-5 w-5 text-blue-600" />
          <span className="text-xs font-medium text-gray-700">블로그 관리</span>
        </Link>
      </div>

      {/* ── 키워드 순위 현황 요약 (기존 - TOP3/TOP10 카운터) ── */}
      {rankedKws.length > 0 && (
        <div className="rounded-xl border bg-white p-4">
          <div className="flex items-center gap-2 text-gray-900 mb-3">
            <TrendingUp className="h-4 w-4 text-emerald-500" />
            <h2 className="text-sm font-semibold">키워드 순위 현황</h2>
          </div>
          <div className="flex gap-3">
            <div className="flex-1 rounded-lg bg-emerald-50 p-2.5 text-center">
              <p className="text-xl font-bold text-emerald-700">{top3Count}</p>
              <p className="text-[10px] text-emerald-600">TOP 3</p>
            </div>
            <div className="flex-1 rounded-lg bg-blue-50 p-2.5 text-center">
              <p className="text-xl font-bold text-blue-700">{top10Count}</p>
              <p className="text-[10px] text-blue-600">TOP 10</p>
            </div>
            <div className="flex-1 rounded-lg bg-gray-50 p-2.5 text-center">
              <p className="text-xl font-bold text-gray-700">{rankedKws.length}</p>
              <p className="text-[10px] text-gray-500">전체 노출</p>
            </div>
          </div>
        </div>
      )}
      ...
      {!healthScore && data.latestAnalysis?.marketing_score != null && (
        <div className="rounded-xl border bg-white p-4">
          <div className="flex items-center gap-2 text-gray-900 mb-3">
            ...
            <h2 className="text-sm font-semibold">마케팅 종합 점수</h2>
          ...
          <div className="flex items-center gap-6 mb-3">
            <div className="relative flex items-center justify-center h-16 w-16 shrink-0">
              ...
              <span className="absolute text-base font-bold text-gray-900">{data.latestAnalysis.marketing_score}</span>
            </div>
                ...
                return areas.map(({ key, label, max, color }) => {
                  ...
                  return (
                    <div key={key} className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-medium text-gray-600 w-10 text-right">{score}/{areaMax}</span>
                    </div>
                  );
                });
              })()}
